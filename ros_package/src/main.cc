...
public:
    ...
    static void robotControlHandPositionCallback(const dynamic_biped::robotHandPosition::ConstPtr &msg)
    {
        ...
        auto isInRange = [](const std::vector<uint8_t>& positions) {
            return std::all_of(positions.begin(), positions.end(), [](uint8_t pos) { return pos >= 0 && pos <= 100; });
        };

        if (!isInRange(msg->left_hand_position) || !isInRange(msg->right_hand_position)) {
            ROS_WARN("Invalid hand positions value. All positions value must be in the range [0, 100].");
            return;
        }
