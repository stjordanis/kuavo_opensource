...
namespace HighlyDynamic
{
    ...
    public:
        ...
        void velocityCommand(VelocityData velocityData);   // 接收速度控制输入
        ...
        void changeArmPoses(std::vector<int> index_vec,int replay_times =1);

        void changeHandArmPose(int index);
        std::pair<int, int> changeHandArmPoseDelay(int index);

        void changeWalkingArmPose(int index);//改变行走时手部的固定姿势
    ...
    private:
        ...
        CSVParamLoader *pose_param_ptr, *pose_hand_arm_param_ptr, *pose_hand_arm_delay_ptr;
        Eigen::VectorXd current_arm_pose_degree, walk_arm_pose, walk_hand_pose, walk_hand_arm_pose, walk_hand_arm_delay_pose;
